...
$(function() {
	...
	// $(".amp-formulario-buscador-titles").eq(0).css("margin-left", "238px");
	// // $("#stationDest").css("display","none");
	// }
	// else if (this.value == 'tsucursal2') {
	// $(".visible").css("display","none");
	// $(".oculta").css("display","inline");
	// $(".amp-formulario-buscador-titles").eq(0).css("margin-left", "401px");
	// //$("#stationDest").css("display","");
	// }
	// });

	$('input[type=checkbox][name=misma-oficina]').change(
			function() {
				if ($(this).is(':checked')) {
...
function comprobar() {
	...
	if (($(".oculta").css('display') != 'none')
			&& (stationori === undefined || stationori == "")) {
		jAlert("Debes introducir una localidad origen", "Alerta");
		return false;
	}

	if (($(".oculta").css('display') != 'none')
			&& (stationdes === undefined || stationdes == "")) {
...
function compararHoraActual_HInicio() {
	...
	if (mintot_select <= totminAct) {
		jAlert(
				"Error. La hora de recogida no puede ser inferior a la Hora Actual.",
				"Error");
		// inicializarHoraInicio();
		HoraRecogidaValida();
		return false;
	}
	// C�digo que muestra mensaje por si se ha codigo origen con menos de dos
	// horas de antelaci�n
	// else if(dif < 120)
	// {
	// jAlert("Error. La hora m�nima de recogida es de 2 horas m�s, de la hora
	// actual.Se coloca hora mas cercana valida","Error");
	// //inicializarHoraInicio();
	// HoraRecogidaValida();
	// return false;
	// }
